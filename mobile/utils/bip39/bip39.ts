...
export function mnemonicToEntropy(mnemonic: string, wordslist?: string[]): string {
...
export function entropyToMnemonic(entropy: string, wordslist?: string[]): string {
    const wordlist = wordslist || EN_WORDLIST;
...
export function generateMnemonic(strength: number = 128, wordlist?: any): string {
    const randomBytesBuffer = Buffer.from(crypto.randomBytes(strength / 8));
